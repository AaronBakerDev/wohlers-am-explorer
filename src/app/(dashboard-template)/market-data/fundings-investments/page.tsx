...
export default function FundingsInvestmentsPage() {
  ...
  const dataset = 'fundings-investments'
  const config = {
    name: 'Fundings & Investments',
    description: 'Investment rounds and funding data for additive manufacturing companies'
  }
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mb-4 mt-4">
              <Badge variant="secondary">
                {loading ? 'Loading...' : 
                 csvData.length > 1 ? `${csvData.length - 1} of ${totalRows.toLocaleString()} investment records` :
        ...
        {!loading && !error && csvData.length > 0 && (
          <div>
            {view === 'table' ? (
              ...
              <InvestmentAnalysisLayout data={csvData} dataset={dataset} />
            )}
          </div>
        )}
