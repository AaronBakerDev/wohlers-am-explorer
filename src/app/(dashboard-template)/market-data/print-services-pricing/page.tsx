...
export default function PrintServicesPricingPage() {
  ...
  const dataset = 'print-services-pricing'
  const config = {
    name: 'Print Services Pricing',
    description: 'Pricing data for additive manufacturing services, including costs and lead times'
  }
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const apiUrl = `/api/market-data/${dataset}?all=true`
        ...
        {!loading && !error && csvData.length > 0 && (
          <div>
            {view === 'table' ? (
              ...
              <PricingAnalysisLayout data={csvData} dataset={dataset} />
            )}
          </div>
        )}
